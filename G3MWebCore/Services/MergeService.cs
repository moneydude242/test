...
public static partial class MergeService
{
        ...
        MergeOptions options)
    {
        ...
        else
        {
            ...
            zipOutputPath = Path.Combine(
                Path.GetTempPath(),
                $"merged_{timestamp}.g3mpatch");
        }
        ...
        LogService.Log($"Merge: {patchPaths.Count} patches with original {Path.GetFileName(originalPath)}");
        for (int i = 0; i < patchPaths.Count; i++)
            LogService.Log($"  [{i + 1}] {patchNames[i]}");
        ...
        try
        {
            ...
            for (int i = 0; i < patchPaths.Count; i++)
            {
                ...
                normalizeTasks[idx] = Task.Run(async () =>
                {
                    ...
                    {
                        throw new Exception(
                            $"Failed to normalize patch '{Path.GetFileName(path)}' ({path}): {ex.Message}",
                            ex
                        );
                    }
            ...
            LogService.Log($"Merge complete in {totalSw.Elapsed.TotalSeconds:F1}s: " +
            ...
            if (saveZip) LogService.Log($"  Patch: {zipOutputPath}");
            if (applyData) LogService.Log($"  Applied: {options.ApplyPath}");
            if (logPath != null) LogService.Log($"  Report: {logPath}");
