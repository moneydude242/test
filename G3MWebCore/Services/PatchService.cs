...
public partial class PatchService
{
    ...
    private static void ValidateZipArchiveReadable(string zipPath, string context)
    {
        if (!File.Exists(zipPath))
            throw new Exception($"{context} was not created: {zipPath}");

        var fileInfo = new FileInfo(zipPath);
        if (fileInfo.Length == 0)
            throw new Exception($"{context} is empty: {zipPath}");
        ...
        {
            throw new Exception(
                $"{context} is not a readable ZIP archive ({fileInfo.Length} bytes): {zipPath}. {ex.Message}",
                ex
            );
        }
    }
        ...
        DataFileInfo? precomputedOriginalInfo = null)
    {
        ...
        {
            // Verify by magic bytes — older G3M mods may store raw VCDIFF data under a .g3mpatch extension.
            byte[] probe = new byte[4];
            int bytesRead;
            using (var fs = File.OpenRead(inputPath))
                bytesRead = fs.Read(probe, 0, probe.Length);

            if (bytesRead >= 2 && probe[0] == 0x50 && probe[1] == 0x4B) // PK ZIP magic
            {
                ValidateZipArchiveReadable(
                    inputPath,
                    $"Input patch '{Path.GetFileName(inputPath)}'"
                );
                return inputPath;
            }

            // VCDIFF magic (0xD6 0xC3 0xC4) — treat as a raw xdelta stream despite the extension
            if (bytesRead >= 3 && probe[0] == 0xD6 && probe[1] == 0xC3 && probe[2] == 0xC4)
            {
                LogService.Warning($"Input '{Path.GetFileName(inputPath)}' has .g3mpatch extension but is raw VCDIFF — treating as xdelta.");
                ext = ".xdelta";
            }
            // else: unknown binary with .g3mpatch ext — fall through to the final else { return inputPath }
        }

        string dataFilePath;
        bool isFromXdelta = false;
        ...
        ValidateZipArchiveReadable(
            outputZip,
            $"Converted patch '{Path.GetFileName(inputPath)}'"
        );

        if (isFromXdelta && dataFilePath != inputPath)
