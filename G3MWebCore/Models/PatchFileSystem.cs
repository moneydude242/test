...
public sealed class PatchFileSystem
{
    ...
    {
        return LoadFromBytes(File.ReadAllBytes(zipPath));
    }
    ...
    /// </summary>
    public byte[] SaveToBytes(G3MPatchManifest? manifest = null)
    {
            ...
            leaveOpen: true))
        {
            ...
            {
                var entryPath =
                    $"CodeEntries/{codeName}/{codeName}.gml";
            ...
            {
                var entryPath =
                    $"CodeEntries/{codeName}/{codeName}.asm";
    ...
    public static PatchFileSystem LoadFromBytes(byte[] zipBytes)
    {
        ...
        foreach (var entry in archive.Entries)
        {
            ...
            if (fullName.Equals("g3mpatch.json", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    pfs.Manifest =
                        JsonSerializer.Deserialize<G3MPatchManifest>(bytes);
                }
                catch
                {
                    // Keep compatibility with the existing implementation.
                }
        ...
        pfs.HelpersPrefix =
            hasHelpers
                ? "Helpers"
                : (hasAssetOrder ? "AssetOrder" : "Helpers");

        pfs.BuildDirectoryIndex();

        long totalBytes =
            pfs._files.Values.Sum(b => (long)b.Length);

        Debug.WriteLine(
            $"[PatchFileSystem] Loaded {pfs._files.Count} files + " +
            $"{pfs.GmlEntries.Count} GML + {pfs.AsmEntries.Count} ASM " +
            $"in {sw.Elapsed.TotalSeconds:F1}s " +
            $"({zipBytes.Length / 1024 / 1024}MB ZIP -> " +
            $"{totalBytes / 1024 / 1024}MB memory)");
